				...
				<div class="headBox">
					<el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect" :router="true">
						...
						<el-submenu index="/Share">
							...
							<el-menu-item v-for="(item,index) in classListObj" :key="'class1'+index" @click=queryArticleByCategoryId(item.id,item.name) :index="item.id">{{item.name}}</el-menu-item>
						</el-submenu>
...
export default {
    ...
    queryArticleByCategoryId(categoryId, categoryName){
      this.$router.push({
        path: "/Share?class=" + categoryName,
        name: 'Share',
        params: {
          "classId": categoryId
        }
      });
      // this.bus.$emit("queryArticleByCategoryId", categoryId)
    },
